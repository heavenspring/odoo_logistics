{
    ...
    'depends': ['base','mail'],#这是该模块所直接依赖的模块技术名称列表。如果你的模块不依赖于任何其它插件模块，那么应至少添加一个base模块。别忘记包含这个模块所引用的XML ID、视图或模块的定义模型。那样可确保它们以正确的顺序进行加载，避免难以调试的错误。 
    ...
    'website': "http://hq.ksmyzbzx.com",#这个 URL 可供人们访问来了解模块或作者的更多信息
    'version': '16.0.1',#这是该模块的版本号。可供Odoo应用商店用于检测已安装模块的新版本。如果版本号没有以Odoo目标版本号（如14.0）开头，会进行自动添加。但是，如果你显式的声明Odoo目标版本号信息量会更充足，比如用14.0.1.0.0或14.0.1.0来替代1.0.0或1.0。 
}
